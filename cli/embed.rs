...
async fn run_internal(flags: Arc<Flags>) -> Result<i32, AnyError> {
  tools::run::check_permission_before_script(&flags);

  let factory = CliFactory::from_flags(flags);
  let cli_options = factory.cli_options()?;
  let main_module = cli_options.resolve_main_module()?;
  if main_module.scheme() == "npm" {
    tools::run::set_npm_user_agent();
  }

  tools::run::maybe_npm_install(&factory).await?;
  ...
  let global = worker.execute_script_static(
    deno_core::located_script_name!(),
    "test({ content: 'Hello from Rust!'})",
  )?;
...
pub fn run(args: Vec<std::ffi::OsString>) -> Result<i32, AnyError> {
  let future = async move {
    let flags = resolve_flags_and_init(args)?;
    let flags = Arc::new(flags);
    let handle = spawn_subcommand(async move { run_internal(flags).await });
...
fn test_run() {
  let args = vec![
    std::ffi::OsString::from("denocliembed"),
    std::ffi::OsString::from("../test_embed/example.tsx"),
  ];
  let result = run(args);

  // This line will print
  //   Result: Ok(String("<div>Hello world! I come from Rust :-)</div>"))
